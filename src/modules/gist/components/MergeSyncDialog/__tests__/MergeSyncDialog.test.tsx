...
describe('MergeSyncDialog', () => {
  ...
  const removedInGist = [{ name: 'moment', addedAt: '2025-01-03T00:00:00.000Z' }]
  const emptyMaintainers = { addedMaintainersInGist: [], removedMaintainersInGist: [] }

  it('shows packages added on another device', () => {
    render(<MergeSyncDialog delta={{ addedInGist, removedInGist: [], ...emptyMaintainers }} onKeepAll={jest.fn()} onReplaceWithLocal={jest.fn()} />)
  ...
  it('calls onKeepAll when Keep all is clicked', () => {
    ...
    render(<MergeSyncDialog delta={{ addedInGist, removedInGist, ...emptyMaintainers }} onKeepAll={onKeepAll} onReplaceWithLocal={jest.fn()} />)
  ...
  it('calls onReplaceWithLocal when Replace with current is clicked', () => {
    ...
    render(<MergeSyncDialog delta={{ addedInGist, removedInGist, ...emptyMaintainers }} onKeepAll={jest.fn()} onReplaceWithLocal={onReplaceWithLocal} />)
