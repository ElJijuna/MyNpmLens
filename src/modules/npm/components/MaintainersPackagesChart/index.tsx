...
export const MaintainersPackagesChart = ({ usernames }: MaintainersPackagesChartProps) => {
  ...
  const results = useQueries({
    queries: usernames.map((username) => ({
      ...
      queryFn: ({ signal }: { signal: AbortSignal }) =>
        client.maintainer(username).packages({ size: 1, from: 0 }, signal),
  ...
  const data = usernames
    .map((username, index) => ({
      ...
      hasData: results[index].data !== null && results[index].data !== undefined,
    }))
