...
async function validateMaintainer(username: string): Promise<void> {
  ...
  if (!res.ok) {
    throw new ProxyError('npm-registry', res.status, `npm registry responded with ${res.status}`);
  }
  const data: { total: number } = await res.json();
  if (data.total === 0) {
    throw new ProxyError('npm-registry', 404, `Maintainer "${username}" not found on npm`);
  }
}
