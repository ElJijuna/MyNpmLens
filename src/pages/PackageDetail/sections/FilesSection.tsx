...
export function FilesSection({ name, version }: FilesSectionProps) {
  ...
  const title = data ? `${t('packageDetail.files')} (${files.length})` : t('packageDetail.files')

  return (
    <SectionCard title={title} isLoading={isPending} error={error as Error | null}>
      {data && files.length === 0 && <Text color="dim">{t('packageDetail.noFiles')}</Text>}
      {files.length > 0 && (
            ...
            <div style={{ ...filesGridStyle, marginBottom: '0.5rem' }}>
              {[...dirs].sort().map((dir) => (
                <WrapBox key={dir} align="center" childSpacing={12} style={fileRowStyle}>
                  ...
                  <Text variant="caption" color="dim" style={{ fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>
                    {dir}/
                  </Text>
                </WrapBox>
              ))}
            ...
            {files.map((f) => {
              ...
              return (
                <WrapBox key={f.path} align="center" childSpacing={12} style={fileRowStyle}>
                  <Icon icon={iconData ? { path: iconData.path } : Document} size="sm" style={iconData?.hex ? { color: `#${iconData.hex}` } : undefined} />
