        ...
        {entries.map(([name, range]) => {
          ...
          return (
            ...
            >
              ...
              <Text variant="caption" style={{ fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>
                {name}
              </Text>
        ...
        <WrapBox childSpacing={6} align="center" style={{ marginBottom: '1rem' }}>
          {directCount > 0 && (
            <Badge variant="accent">
              {directCount} {t('packageDetail.depsDirectCount')}
            </Badge>
          )}
          {transitiveCount > 0 && (
            <Badge variant="neutral">
              {transitiveCount} {t('packageDetail.depsTransitiveCount')}
            </Badge>
          )}
        </WrapBox>
      )}

      {data && !hasDeps && <Text color="dim">{t('packageDetail.noDependencies')}</Text>}
      {data && hasDeps && (
        <Box orientation="vertical" spacing={20}>
          <DepGroup label={t('packageDetail.depsRuntime')} variant="accent" deps={data.dependencies ?? {}} onNavigate={goToPackage} />
          <DepGroup label={t('packageDetail.depsPeer')} variant="warning" deps={data.peerDependencies ?? {}} onNavigate={goToPackage} />
          <DepGroup label={t('packageDetail.depsDev')} variant="neutral" deps={data.devDependencies ?? {}} onNavigate={goToPackage} />
          <DepGroup label={t('packageDetail.depsOptional')} variant="neutral" deps={data.optionalDependencies ?? {}} onNavigate={goToPackage} />
        </Box>
      )}
    </SectionCard>
  )
