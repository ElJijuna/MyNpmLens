...
export function ProfilePage() {
  ...
  const hookOpts = { token: user?.githubToken, enabled: !!login && !!user?.githubToken }
          ...
          <WrapBox justify="space-between" align="start">
            <Box orientation="horizontal" spacing={16} style={{ alignItems: 'flex-start' }}>
              ...
              <Box spacing={4}>
                ...
                {ghUser?.login && <Text variant="caption" color="dim">@{ghUser.login}</Text>}
                {ghUser?.bio && <Text>{ghUser.bio}</Text>}
                <WrapBox childSpacing={12}>
                  {ghUser?.company && <Text variant="caption" color="dim">{ghUser.company}</Text>}
                  {ghUser?.location && <Text variant="caption" color="dim">{ghUser.location}</Text>}
                  {memberYear && (
    ...
    <Card padding="md">
      <Box spacing={6}>
        <WrapBox justify="space-between" align="start" childSpacing={8}>
          ...
          >
            <Text variant="body" style={{ wordBreak: 'break-word' }}>{repo.name}</Text>
          </a>
        ...
        {repo.description && (
          <Text variant="caption" color="dim" style={{
            display: '-webkit-box',
            WebkitLineClamp: 2,
